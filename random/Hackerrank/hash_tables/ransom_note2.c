...
int					main()
{
	...
	for(int magazine_i = 0; magazine_i < m; magazine_i++)
	{
		...
		DictInsert(d_mag, magazine[magazine_i]);
	//	DictIncrValue(d_mag, magazine[magazine_i]);
	}
	// take array of words and add each to new dict
	// then implement compare values function		
	...
	for(int ransom_i = 0; ransom_i < n; ransom_i++)
	{
		...
		DictInsert(d_note, ransom[ransom_i]);
	//	DictIncrValue(d_note, ransom[ransom_i]);
	}
